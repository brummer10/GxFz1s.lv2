...
bool gx_gui_open_display(gx_maestro_fz1sUI *ui) {
	return true; // STUB
}

void gx_gui_create_window_and_surface(gx_maestro_fz1sUI *ui) {
	// prepare window class
	static TCHAR szClassName[] = TEXT("gx_maestro_fz1sUIClass");
	WNDCLASS wndclass = {0};
...
void gx_gui_register_controller_message(gx_maestro_fz1sUI *ui) {
	// dummy, not required on MSWin
}

void gx_gui_destroy_main_window(gx_maestro_fz1sUI *ui) {
	DestroyWindow(ui->win);
	// safe to use: doesnt unregister if there are still windows of this class
	UnregisterClass(TEXT("gx_maestro_fz1sUIClass"), NULL);
}

void gx_gui_resize_surface(gx_maestro_fz1sUI *ui) {
	RECT rect;
...
void gx_gui_send_controller_event(gx_maestro_fz1sUI *ui, int controller) {
...
void event_handler(gx_maestro_fz1sUI *ui) {
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	gx_maestro_fz1sUI *ui = (gx_maestro_fz1sUI *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
