...
#define GTK_FZ1S_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_FZ1S_KNOB, GtkFz1sKnobPrivate))

G_DEFINE_TYPE(GtkFz1sKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkFz1sKnobPrivate GtkFz1sKnobPrivate;

struct _GtkFz1sKnobPrivate
{
    int knob_x;
    int knob_y;
    int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
    /** check resize **/
    int grow;
    GtkFz1sKnobPrivate *priv = GTK_FZ1S_KNOB_GET_PRIVATE(widget);
    ...
    if (priv->model != 3) {
        ...
        GtkFz1sKnob *knob = GTK_FZ1S_KNOB(widget);
...
{
    g_assert(GTK_IS_FZ1S_KNOB(widget));
    GtkFz1sKnob *knob = GTK_FZ1S_KNOB(widget);
    GtkFz1sKnobPrivate *priv = GTK_FZ1S_KNOB_GET_PRIVATE(knob);
...
{
    GtkFz1sKnob *knob = GTK_FZ1S_KNOB(object);
...
/****************************************************************
 ** init the GtkFz1sKnobClass
 */

static void gtk_knob_class_init (GtkFz1sKnobClass *klass)
{
...
//--------- connect the events with funktions
    ...
    g_type_class_add_private(obj_class, sizeof (GtkFz1sKnobPrivate));
}
...
static void gtk_knob_init (GtkFz1sKnob *knob)
{
...
{
    GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_FZ1S_KNOB, NULL ));
